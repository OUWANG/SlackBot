...
rtm.on(RTM_EVENTS.MESSAGEd, function handleRtmMessage(message) {
      ...
      .then(function({data}) {
        ...
        if (data.result.actionIncomplete) {
          ...
          if (data.result.parameters.invitees) {
            ...
            user.save()
            User.findOne({slackId: user.slackId}, function (userReloaded) {
              console.log('<<<<<<<<< Reloaded user >>>>>>>>>>>>', userReloaded.pending)
            })

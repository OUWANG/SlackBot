...
function getQuery(message, session) {
  ...
  .then(function(user) {
    ...
    if (!user.google) {
      rtm.sendMessage( `Hello,
        This is Shcedule Bot. In order to ... Please visit http://localhost:3000/connect?user=${user._id} to setup google
        calendar`, message.channel);
      return;
    }
    // rtm.sendMessage('Your id is' + user._id, message.channel)

    getQuery(message.text, message.user)
      .then(function({data}) {
